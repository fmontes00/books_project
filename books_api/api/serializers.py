...
class AuthorSerializer(serializers.ModelSerializer):
    ...
    def get_books(self, obj):
        return BaseBookSerializer(obj.books.order_by("title"), many=True).data


class BaseBookSerializer(serializers.ModelSerializer):
    class Meta:
        model = Book
        fields = ("id", "title", "created", "publish_date")
...
class BookSerializer(BaseBookSerializer):
    ...
    class Meta:
        model = Book
        fields = BaseBookSerializer.Meta.fields + ("author", "collaborator")
    ...
    def get_collaborators(self, obj):
        return CollaboratorSerializer(obj.collaborator.all(), many=True).data
